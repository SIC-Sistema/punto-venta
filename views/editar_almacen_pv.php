...
?>
  ...
  <head>
    ...
    $id = $_POST['id'];// POR EL METODO POST RECIBIMOS EL ID DEL ALMACEN
    //REALIZAMOS LA CONSULTA PARA SACAR LA INFORMACION DEL ALMACEN Y ASIGNAMOS EL ARRAY A UNA VARIABLE $datos
    $datos = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `punto_venta_almacenes` WHERE id=$id"));
    ?>
    <script>
      //FUNCION QUE HACE LA ACTUALIZACION DEL ALMACEN (SE ACTIVA AL PRECIONAR UN BOTON)
