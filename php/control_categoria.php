...
switch ($Accion) {
    case 0:  ///////////////           IMPORTANTE               ///////////////
        ...
        //VERIFICAMOS QUE NO HAYA UNA CATEGORIA CON LOS MISMOS DATOS
		if(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM `punto_venta_categorias` WHERE nombre='$Nombre' "))>0){
            ...
            $sql = "INSERT INTO `punto_venta_categorias` (parent_id,nombre, usuario, fecha) 
               VALUES('0','$Nombre','$id_user','$Fecha_hoy')";
    ...
    case 1:  ///////////////           IMPORTANTE               ///////////////
		...
		if ($Texto != "") {
			...
			$sql = "SELECT * FROM `punto_venta_categorias` WHERE  parent_id = 0 and nombre LIKE '%$Texto%' ORDER BY id";	
		}else{//ESTA CONSULTA SE HARA SIEMPRE QUE NO ALLA NADA EN EL BUSCADOR Y GUARDAMOS LA CONSULTA SQL EN UNA VARIABLE $sql...
			$sql = "SELECT * FROM `punto_venta_categorias` WHERE parent_id  = 0";
            ...
            while($categoria = mysqli_fetch_array($consulta)) {
                ...
                $contenido .= '			
		          <tr>
                    ...
                    <td><form method="post" action="../views/subcategorias_pv.php"><input id="id" name="id" type="hidden" value="'.$categoria['id'].'"><button class="btn-floating btn-tiny waves-effect waves-light cyan darken-3"><i class="material-icons">visibility</i></button></form></td>
		          </tr>';
			}//FIN while
        }//FIN else
    ...
    case 3:
        ...
        break;
    case 4:
        $categoryId = $conn->real_escape_string($_POST['valorCategoria']);
        $nombreSubcategoria = $conn->real_escape_string($_POST['valorNombre']);
        $consultaCategoria = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM `punto_venta_categorias` WHERE id = $categoryId"));
        $insertarSubcategoria = "INSERT INTO punto_venta_categorias (parent_id, nombre, nombre_sub, usuario, fecha) 
        VALUES('$categoryId', '".$consultaCategoria['nombre']."', '$nombreSubcategoria', '$id_user', '$Fecha_hoy')";
        if (mysqli_query($conn,$insertarSubcategoria)){
            echo '<script >M.toast({html:"La subcategoria se actualizó con exito.", classes: "rounded"})</script>';	
            echo '<script>recargar_subcategorias()</script>';
        }else{
            echo '<script >M.toast({html:"Ocurrió un error al registrar subcategoría.", classes: "rounded"})</script>';	
        }
    break;
    case 5:
       //CON POST RECIBIMOS UN TEXTO DEL BUSCADOR VACIO O NO de "categorias_punto_venta.php"
       $categroiaId = $conn->real_escape_string($_POST['categoriaId']);
       //VERIFICAMOS SI CONTIENE ALGO DE TEXTO LA VARIABLE
       
        //MOSTRARA LAS CATEGORIAS QUE SE ESTAN BUSCANDO Y GUARDAMOS LA CONSULTA SQL EN UNA VARIABLE $sql......
        $sql = "SELECT * FROM `punto_venta_categorias` WHERE parent_id = $categroiaId";	
       // REALIZAMOS LA CONSULTA A LA BASE DE DATOS MYSQL Y GUARDAMOS EN FORMARTO ARRAY EN UNA VARIABLE $consulta
       $consultaSubCategoria = mysqli_query($conn, $sql);		
       $contenido = '';//CREAMOS UNA VARIABLE VACIA PARA IR LLENANDO CON LA INFORMACION EN FORMATO
       ...
       if (mysqli_num_rows($consultaSubCategoria) == 0) {
           ...
           //La variable $contenido contiene el array que se genera en la consulta, así que obtenemos los datos y los mostramos en un bucle
           //RECORREMOS UNO A UNO LOS ARTICULOS CON EL WHILE
           while($categoria = mysqli_fetch_array($consultaSubCategoria)) {
               //Output
               $contenido .= '			
                 <tr>
                   <td>'.$categoria['nombre_sub'].'</td>
                   <td>'.$categoria['nombre'].'</td>
                   <td><form method="post" action="../views/editar_subcategoria_pv.php"><input id="id" name="id" type="hidden" value="'.$categoria['id'].'"><button class="btn-floating btn-tiny waves-effect waves-light pink"><i class="material-icons">edit</i></button></form></td>
                   <td><a onclick="borrar_subcategoria_pv('.$categoria['id'].')" class="btn btn-floating red darken-1 waves-effect waves-light"><i class="material-icons">delete</i></a></td>
                 </tr>';
           }//FIN while
       }//FIN else
       echo $contenido;// MOSTRAMOS LA INFORMACION HTML
       ...
       case 6:///////////////           IMPORTANTE               ///////////////
        ...
        $Nombre = $conn->real_escape_string($_POST['valorNombre']);    
        //VERIFICAMOS QUE NO HALLA UNA CATEGORIA CON LOS MISMOS DATOS
        if(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM `punto_venta_categorias` WHERE (nombre_sub='$Nombre') AND id = $id"))>0){
            echo '<script >M.toast({html:"Ya se encuentra una subcategoria con el mismo nombre.", classes: "rounded"})</script>';
        }else{
            //CREAMOS LA SENTENCIA SQL PARA HACER LA ACTUALIZACION DE LA INFORMACION DE LA CATEGORIA Y LA GUARDAMOS EN UNA VARIABLE
    		$sql = "UPDATE `punto_venta_categorias` SET nombre_sub = '$Nombre', usuario = '$id_user', fecha= '$Fecha_hoy' WHERE id = '$id'";
            //VERIFICAMOS QUE LA SENTECIA FUE EJECUTADA CON EXITO!
    		if(mysqli_query($conn, $sql)){
    			echo '<script >M.toast({html:"La categoria se actualizó con exito.", classes: "rounded"})</script>';	
                echo '<script>recargar_categoria()</script>';// REDIRECCIONAMOS (FUNCION ESTA EN ARCHIVO modals.php)
        ...
        case 7:
            ...
            //CON POST RECIBIMOS LA VARIABLE DEL BOTON POR EL SCRIPT DE "subcategorias_pv.php" QUE NESECITAMOS PARA BORRAR
            $id = $conn->real_escape_string($_POST['id']);
            #SELECCIONAMOS LA INFORMACION A BORRAR
            $categoria = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `punto_venta_categorias` WHERE id = $id"));
            #CREAMOS EL SQL DE LA INSERCION A LA TABLA  `pv_borrar_categorias` PARA NO PERDER INFORMACION
            $sql = "INSERT INTO `pv_borrar_categorias` (id_categoria, parent_id, nombre_sub, nombre, registro, borro, fecha_borro) 
                    VALUES($id, '".$categoria['parent_id']."','".$categoria['nombre_sub']."','".$categoria['nombre']."', '".$categoria['usuario']."', '$id_user','$Fecha_hoy')";
            //VERIFICAMOS QUE LA SENTECIA FUE EJECUTADA CON EXITO!
            if(mysqli_query($conn, $sql)){
            //SI DE CREA LA INSERCION PROCEDEMOS A BORRRAR DE LA TABLA `punto_venta_categorias`
            #VERIFICAMOS QUE SE BORRE CORRECTAMENTE LA CATEGORIA DE `punto_venta_categorias`
            if(mysqli_query($conn, "DELETE FROM `punto_venta_categorias` WHERE `punto_venta_categorias`.`id` = $id")){
                #SI ES ELIMINADO MANDAR MSJ CON ALERTA
                    echo '<script >M.toast({html:"Subcategoria borrada con exito.", classes: "rounded"})</script>';
                    echo '<script>recargar_categoria()</script>';// REDIRECCIONAMOS (FUNCION ESTA EN ARCHIVO modals.php)
            }else{
                #SI NO ES BORRADO MANDAR UN MSJ CON ALERTA
                    echo "<script >M.toast({html: 'Ha ocurrido un error.', classes: 'rounded'});/script>";
